(() => {
    ...
    if (script_form) {
        const firstNameInput = script_form.querySelector(".form__input-first-name");
        const lastNameInput = script_form.querySelector(".form__input-last-name");
        const emailInput = script_form.querySelector(".form__input-email");
        const zipInput = script_form.querySelector(".form__input-zip");
        const checkboxInput = script_form.querySelector(".form__checkbox input");
        zipInput.addEventListener("input", (function(event) {
            const maxLength = 5;
            if (zipInput.value.length > maxLength) zipInput.value = zipInput.value.slice(0, maxLength);
        }));
        script_form.addEventListener("submit", (function(event) {
            ...
            if (!zipRegex.test(zipValue)) {
                ...
                errorElement.textContent = "Please enter a valid zip/postal code, 5 characters";
                zipInput.parentNode.appendChild(errorElement);
            } else if (zipValue.length > 5) {
                isValid = false;
                const errorElement = document.createElement("p");
                errorElement.classList.add("error-message");
                errorElement.textContent = "Zip/postal code should not exceed 5 characters";
                zipInput.parentNode.appendChild(errorElement);
            }
            if (!checkboxInput.checked) isValid = false;
            if (isValid) script_form.submit();
        }));
    }
    window["FLS"] = true;
    isWebp();
